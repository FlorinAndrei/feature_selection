...
class CmaEsSampler(BaseSampler):
    ...
    def __init__(
        ...
        *,
        ...
        if self._use_separable_cma:
            warnings.warn(
                "`use_separable_cma` option is an experimental feature." " The interface can change in the future.",
                ExperimentalWarning,
            )

        if self._source_trials is not None:
            warnings.warn(
                "`source_trials` option is an experimental feature." " The interface can change in the future.",
                ExperimentalWarning,
            )
        ...
        if self._lr_adapt:
            warnings.warn(
                "`lr_adapt` option is an experimental feature." " The interface can change in the future.",
                ExperimentalWarning,
            )
    ...
    def infer_relative_search_space(
        ...
        warnings.warn(f'relative search space: {search_space}')
    ...
    def sample_relative(
        ...
        warnings.warn(f'len(trans.bounds)={len(trans.bounds)}')
        warnings.warn(f'_initial_popsize={self._initial_popsize}')
        ...
        if optimizer is None:
            warnings.warn(f'_init_optimizer')
        ...
        if optimizer.dim != len(trans.bounds):
            if self._warn_independent_sampling:
                _logger.warning(
                    ...
                    "`{}` is used instead of `CmaEsSampler`.".format(self._independent_sampler.__class__.__name__)
                )
                self._warn_independent_sampling = False
            return {}
        ...
        if len(solution_trials) >= popsize:
            ...
            if self._restart_strategy == "bipop" and optimizer.should_stop():
                ...
                if small_n_eval < large_n_eval:
                    ...
                    popsize = math.floor(
                        self._initial_popsize * popsize_multiplier ** (self._cma_rng.rng.uniform() ** 2)
                    )
                else:
                    poptype = "large"
        ...
        warnings.warn(f'sample_relative external values: {external_values}')
    ...
    def _init_optimizer(
        ...
        if self._source_trials is None:
            ...
            source_solutions = [
                ...
                if t.state in expected_states and _is_compatible_search_space(trans, t.distributions)
            ]
        ...
        warnings.warn(f'sigma0={sigma0}, _EPS={_EPS}')
        sigma0 = max(sigma0, _EPS)

        if self._use_separable_cma:
            warnings.warn(f'use SepCMA')
            return cmaes.SepCMA(
                mean=mean,
                sigma=sigma0,
                bounds=trans.bounds,
        ...
        if self._with_margin:
            ...
            warnings.warn(f'use CMAwM')
            cmawm_seed = self._cma_rng.rng.randint(1, 2**31 - 2)
            warnings.warn(
                f'mean={mean[0]}, sigma={sigma0}, bounds={trans.bounds[0]}, steps={steps[0]}, cov={cov}, seed={cmawm_seed}, n_max_resampling={10 * n_dimension}, population_size={population_size}'
            )
            return cmaes.CMAwM(
                mean=mean,
                sigma=sigma0,
    ...
    def sample_independent(
        ...
        ret = self._independent_sampler.sample_independent(study, trial, param_name, param_distribution)
        warnings.warn(f'sample_independent ret: {ret}')
        return ret
